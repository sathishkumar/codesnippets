...
if [ $LEVEL_STARTED == true -o  "$DPA_LEVEL_START" = "2" ]; then
	if [ "$PROD_SET_ID" != *"$ERROR"* -a "$CATALOG_ID" != *"$ERROR"* -a "$PROD_AUD" != *"$ERROR"* ]; then
		...
		echo "CATALOG_ID, PROD_SET_ID and PROD_AUD are required to start from Campaign level"
		exit 0;
	fi # Required fields check
fi # Level start ends here for Campaign

if [ $LEVEL_STARTED == true -o  "$DPA_LEVEL_START" = "3" ]; then
	if [ $PROD_SET_ID != "" -a  $PROD_AUD != "" -a $CAMP_ID != "" -a $ADSET_ID != "" ]; then
		LEVEL_STARTED=true
		DO_ADS
	else
		echo "PROD_SET_ID, PROD_AUD, CAMP_ID and ADSET_ID are required to start from Creative level"
		exit 0;
	fi # Required fields check
